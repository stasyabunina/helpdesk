...
export default class HelpDesk {
  ...
  createTicketList() {
    if (this.ticketList.length !== 0 && this.ticketList !== undefined) {
      this.ticketList.forEach((item) => {
        ...
        this.deleteTicketModalEventListener(ticketElement.deleteBtn, ticketElement.element, ticketElement);
        this.editTicketModalEventListener(ticketElement.editBtn, ticketElement.element, ticketElement.title, ticketElement.description, ticketElement);
      })
    }
  }
  ...
  addTicketModalEventListener() {
    ...
    addTicketModalBtn.addEventListener('click', () => {
      ...
      if (addTicketModal.checkModalForm === true) {
        this.executeBtnEventListener(addTicketModal.modal, addTicketModal.form, addTicketModal);
      }
    })
  }
  ...
  deleteTicketModalEventListener(btn, element, id) {
    btn.addEventListener('click', () => {
      ...
      if (deleteTicketModal.checkModalForm === false) {
        this.executeBtnEventListener(deleteTicketModal.modal, deleteTicketModal.executeBtn, deleteTicketModal, id);
      }
    })
  }
  ...
  editTicketModalEventListener(btn, element, ticketTitle, ticketDesc, id) {
    btn.addEventListener('click', () => {
      const editTicketModal = new Modal('edit', true, element, ticketTitle, ticketDesc);

      editTicketModal.createModal('Изменить тикет');

      if (editTicketModal.checkModalForm === true) {
        this.executeBtnEventListener(editTicketModal.modal, editTicketModal.form, editTicketModal, id);
      }
    })
  }
  ...
  executeBtnEventListener(modalElement, element, modal, id) {
    ...
    element.addEventListener(type, (e) => {
      ...
      if (modal.modalName === 'add') {
        if (modal.titleInput.value.trim() === '' || modal.titleInput.value.length > 60) {
        ...
        if (modal.titleInput.value.trim() === '' || modal.titleInput.value.length > 60) {
          ...
          modal.ticket.querySelector('.ticket__title').textContent = modal.titleInput.value;
          id.ticket.name = modal.titleInput.value;
          if (document.querySelector('.modal__error')) {
            document.querySelector('.modal__error').remove();
          }

          const descWrapper = modal.ticket.querySelector('.ticket__desc-wrapper');

          if (modal.descInput.value.trim() === '' && descWrapper.querySelector('p').classList.contains('ticket__desc')) {
            modal.ticket.querySelector('.ticket__desc').textContent = 'Описание отсутствует.';
            modal.ticket.querySelector('.ticket__desc').className = 'ticket__no-desc';
            id.ticket.description = '';
          } else if (modal.descInput.value.trim() === '' && descWrapper.querySelector('p').classList.contains('ticket__no-desc')) {
            modal.ticket.querySelector('.ticket__no-desc').textContent = 'Описание отсутствует.';
            id.ticket.description = '';
          } else if (descWrapper.querySelector('p').classList.contains('ticket__no-desc') && modal.descInput.value.trim() !== '') {
            descWrapper.querySelector('p').className = 'ticket__desc';
            modal.ticket.querySelector('.ticket__desc').textContent = modal.descInput.value;
            id.ticket.description = modal.descInput.value;
          } else if (descWrapper.querySelector('p').classList.contains('ticket__desc') && modal.descInput.value.trim() !== '') {
            modal.ticket.querySelector('.ticket__desc').textContent = modal.descInput.value;
            id.ticket.description = modal.descInput.value;
          }
  ...
  async createNewTicket(modal) {
    ...
    const data = { id: undefined, name: modal.titleInput.value, description: modal.descInput.value, status: false, created: newData };
    ...
    this.deleteTicketModalEventListener(newTicket.deleteBtn, newTicket.element, newTicket);
    this.editTicketModalEventListener(newTicket.editBtn, newTicket.element, newTicket.title, newTicket.description, newTicket);
  }
  ...
  checkTicketsLength() {
    ...
    if (document.querySelectorAll('.desk__item').length === 0) {
      ...
      text.textContent = 'У вас нет тикетов. Вы можете создать один, нажав на кнопку "Добавить тикет".';
      text.classList.add('no-tickets-text');

      document.querySelector('.desk__list-wrapper').append(text);
      document.querySelector('.desk__list-wrapper').style.justifyContent = 'center';
    }
    ...
    if (document.querySelectorAll('.desk__item').length !== 0 && document.querySelector('.no-tickets-text')) {
      document.querySelector('.no-tickets-text').remove();
    }

    if (document.querySelectorAll('.desk__item').length <= 2 && document.querySelectorAll('.desk__item').length > 0) {
